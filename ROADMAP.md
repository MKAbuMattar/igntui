...
| 15 | **No release pipeline.** Publishing to PyPI is manual, which is the highest-impact thing in this repo to get wrong: PyPI forbids re-uploading a version, so a mistake burns the number permanently. `scripts/release_version.py` now does the bookkeeping; a workflow that prepares on a labelled pull request and publishes only on merge is the other half. | planned |
...
| 19 | **`_load_persistent_cache` reads every cache file at startup.** *(Partly relieved by item 22: `igntui cache info` no longer builds a second manager and re-reads them all a second time.)* Every `.cache` file in the cache directory is opened and JSON-parsed in `CacheManager.__init__`, which runs on import of `core/api` via the `config`/`GitIgnoreAPI` chain — so even `igntui --version` pays for it. Lazy-load on first `get()` instead. | planned |
