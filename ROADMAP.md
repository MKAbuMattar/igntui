...
| 16 | **Branch protection did not require the checks** — in fact `main` had no protection at all, while a merge to it publishes to PyPI. | ✅ done — the four CI contexts are required, branches must be up to date before merging, force-push and deletion are blocked. Admin bypass is left on deliberately. Add `test (py3.14)` to the required list now that it exists |
...
| 20 | **Disk cache writes were not atomic.** `_save_disk_cache` opened the target with `"w"` — truncating it — and only then serialised. A crash, a full disk, or an unserialisable value destroyed the entry that was already there. Verified against the old code: writing a bad value over a good one left `get()` returning `None`. Now a temp file in the same directory plus `os.replace`, so a reader sees the old entry or the new one and never half of either. | ✅ done |
| 21 | **`windows-curses` and the supported Python range.** The worry was that an unbounded marker could resolve to a version with no wheel for the running interpreter. Checked against PyPI: `windows-curses` 2.4.2 ships wheels for cp39 through **cp314**, so there is no gap for anything igntui supports and **no cap is warranted** — adding one would create the failure it was meant to prevent. What the check did turn up: the floor was `>=2.4.1`, which predates cp314 wheels, and igntui itself runs 214/214 tests green on Python 3.14 while neither claiming nor testing it. Floor raised to 2.4.2, 3.14 added to the CI matrix and the classifiers. | ✅ done |
